...
- **Linting**: ESLint flat config + `@typescript-eslint` + `eslint-plugin-astro` + `eslint-plugin-better-tailwindcss` (Tailwind v4 class validation: ordering, canonical forms, unknown class detection; `enforce-consistent-line-wrapping` disabled); markdownlint-cli2 for Markdown (MD013 disabled).
