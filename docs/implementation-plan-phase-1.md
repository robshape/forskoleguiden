...
Capture the completed Phase A visual baseline before starting Step 4 implementation work. This step establishes the shared styling foundation that later functional steps build on.

- Define Tailwind v4 theme tokens in `src/styles/global.css` for core colors, surfaces, borders, layout width, and tray shadow.
- Apply shell styling in `BaseLayout.astro`, `Nav.astro`, and `Footer.astro` so the static structure reflects the approved baseline.
- Add base interactive defaults in `global.css` (focus-visible, links, and button interaction behavior) for consistent keyboard and pointer states.
- Ensure the layout viewport meta is compatible with safe-area tray work in Step 5.3 by using `viewport-fit=cover`.

**Test:** Run `pnpm build`. Inspect `dist/sv/index.html` and confirm the shell renders with styled header/main/footer structure and no regressions from Steps 3.1-3.4. Verify the BaseLayout viewport meta includes `viewport-fit=cover` to support Step 5.3 safe-area tray behavior.

---

## Step 4: Directory Page

> **Note (implementation order):** In Step 4, implement component styling together with functionality. Do not defer card/list/sort visual work to a later step.
...
#### Visual design (Step 4.2, homepage)

Reference: `docs/mockups/homepage.svg`.

- Card container: white surface, subtle border, rounded corners, and balanced inner padding.
- Name/link treatment: preschool name uses prominent heading typography and remains clearly identifiable as a link.
- Metadata line: address/operator/profile metadata uses smaller muted text with compact spacing under the name.
- Score badge: use a circular percentage badge with a two-line text block (`Andel instämmande` + `Helhetsbedömning`), and apply threshold-based badge colors (for example high = green, mid = amber, low = neutral).
- Compare button states: unselected state uses muted secondary styling with `Jämför`; selected state uses a high-contrast primary style with checkmark and Swedish selected label (`Tillagd` or `Tillagd ✓`).
...
- Heading + count: render the directory heading and total count as a single prominent row (e.g., `Förskolor (142)`).
...
#### Visual design (Step 4.4, homepage)

Reference: `docs/mockups/homepage.svg`.

- Use a two-option tab/segmented-button control for ranking vs alphabetical sorting.
...
#### Visual design (Step 5.2, homepage)

Reference: `docs/mockups/homepage.svg`.

- The compare button's unselected/selected states must use the same visual contract defined in Step 4.2.
- Keep label, radius, padding, and contrast behavior consistent between card placeholder state (Step 4.2) and interactive island state (Step 5.2).
- Preserve Swedish state labels (`Jämför` and `Tillagd`/`Tillagd ✓`) as the source-of-truth copy for Phase 1.
...
#### Visual design (Step 5.3, homepage)

Reference: `docs/mockups/homepage.svg`.

- Tray container: fixed to bottom viewport edge, full width, white surface, top border/shadow separation, and mobile-first spacing.
- Safe-area behavior: include bottom padding that accounts for `env(safe-area-inset-bottom)`; this depends on Step 3.5 using `viewport-fit=cover` in the viewport meta.
- Left content: show selected-count as the primary line and selected preschool names/summary as secondary muted text.
- Right content: render a prominent primary CTA (`Visa jämförelse`) with pill/button styling; keep `Rensa` visually secondary but clearly actionable.
...
#### Visual design (Step 6.1, preschool details)

Reference: `docs/mockups/preschool-details.svg`.

- Back navigation treatment: place a compact back link/button at the top of the content area with clear hit area and muted secondary styling.
- Title and metadata styling: render the preschool title as the dominant heading, with address/operator/year metadata directly below in smaller muted text.
- Action button row styling: group page actions (including compare) in a single horizontal row with consistent button height/radius and clear primary-vs-secondary emphasis.
- Section heading/subtitle styling: render "Helhetsbedömning" as a section heading with a short subtitle/intro line using secondary text styling.
- If shortlist controls appear in the mockup, treat them as a visual placeholder only in Phase 1 (no shortlist functionality in this step).
...
#### Visual design (Step 6.2, preschool details)

Reference: `docs/mockups/preschool-details.svg`.

- Per-question card surface: render each question block as a separate card with white surface, subtle border, rounded corners, and consistent inner spacing.
- Text hierarchy: keep question text as primary, then response rows/labels as secondary, and percentages as right-aligned numeric emphasis.
- Chart/legend arrangement: if a detail-page chart is included, place it below the response list with its legend adjacent or directly under it, matching the detail mockup rhythm.
...
#### Visual design (Step 7.2, comparison)

Reference: `docs/mockups/comparison-view.svg`.

- Back/header treatment: use a compact back affordance and page header block consistent with the comparison mockup entry pattern.
- Sticky comparison column header: keep the selected preschool header row sticky during vertical scroll so school labels remain visible while reading sections.
- Summary banner style: render the summary area as a distinct full-width banner/card with clear contrast from the table body.
- Per-question section card structure and divider: group each compared question in its own section card and separate sections with visible dividers.
- Deterministic summary rendering and final summary styling are finalized in Step 9; Step 7.2 only establishes the container/presentation slot.
...
#### Visual design (Step 7.4, comparison)
...
#### Visual design (Step 8.1, comparison charts)

- Preserve the canonical 5-category response model as the source-of-truth for comparison charts and comparison tables.
- A derived 3-segment presentation (agree/neutral/disagree) is allowed on the detail page only if explicitly implemented there; do not replace the canonical 5-category model in comparison.
- Keep legend styling aligned to mockups as dot-label pairs for each category.
- Keep chart-adjacent source/attribution placement visually compatible with Step 10, where attribution copy and final attribution styling are defined.
