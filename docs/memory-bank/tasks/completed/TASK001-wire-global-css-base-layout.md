...
| --- | --------------------------------------------------- | -------- | ---------- | ----------------------------------------------------------------------------------- |
| 1.0 | Add e2e shell test for semantic landmarks (Phase 1) | Complete | 2026-03-02 | Added `tests/e2e/layout-shell-accessibility.spec.ts` with `html/header/main/footer` |
...
- Created new targeted e2e test `tests/e2e/layout-shell-accessibility.spec.ts` for Phase 1 shell criteria.
- Performed fail-first run with `pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts`; first run failed because `/sv/` was missing a `<header>` landmark.
...
- Re-ran with `pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts` and confirmed pass.
- Re-ran with `pnpm build && pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts` and confirmed pass.
...
- Removed redundant dead-code null guard from `tests/e2e/layout-shell-accessibility.spec.ts`.
- Verified fixes with fail-first then green validation: `pnpm test`, `pnpm lint`, `pnpm lint:md`, `pnpm format`, and `pnpm build && pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts`.
...
- Added fail-first nav contracts in `tests/unit/sv-index-layout.test.ts` and `tests/e2e/layout-shell-accessibility.spec.ts`.
...
- Completed Phase 3 validation with green gates: `pnpm lint`, `pnpm lint:md`, `pnpm format`, `pnpm test`, and `pnpm build && CI=1 pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts`.
...
- Consolidated duplicated e2e shell checks into one longer scenario in `tests/e2e/layout-shell-accessibility.spec.ts` (single `page.goto('/sv/')`).
- Verified fail-first then green with: `pnpm test tests/unit/sv-index-layout.test.ts` (fail then pass), `CI=1 pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts` (fail then pass), plus required gates `pnpm lint`, `pnpm lint:md`, `pnpm format`, `pnpm test`.
...
- Extended `tests/e2e/layout-shell-accessibility.spec.ts` with Step 3.3 footer runtime assertions for visible attribution text and Malmö source link `href` on `/sv/`.
- Ran targeted red validation: `CI=1 pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts` (failed while `playwright` preview served stale build output).
- Rebuilt and re-ran targeted validation: `pnpm build && CI=1 pnpm test:e2e tests/e2e/layout-shell-accessibility.spec.ts` (passed).
